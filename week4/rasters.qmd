...
# Find out number of layers in our raster object 

nlyr(landsat)

# Find out names of layers 
...
# This is the position of cell id 468
zion_elevation[2, 3]
```
...
id <- terra::cellFromXY(zion_elevation, xy = point)
# index to cell
zion_elevation[id]
...
```{r}
# Check that extent actually changed 
...
```{r}
# Create a raster mask of same resolution and extent 

rmask <- zion_elevation

# Set all cells with elevation less than 2000 meters
...
```{r}
# Doubles each cell's value 
zion_elevation + zion_elevation

# Square each value in each cell 

zion_elevation^2 

# Take the log of each cells' value 

log(zion_elevation)

# Determines whether each cell has a value greater than 5 
...
elevation_focal <- focal(zion_elevation, 
    ...
    fun = min) # Function to map values 
