...
```{r}
# Updatae park boundary object name to clarify its to clarify that it's a polygon
...
```{r}
# Create low resolution template raster 
template_raster_low <- rast(ext(park_polygon), resolution = .05, crs = st_crs(park_polygon))

# Create high resolution template raster

template_raster_high <- rast(ext(park_polygon), resolution = .01, crs = st_crs(park_polygon))

# Rasterize to low resolution template raster 
polygon_raster_low <- rasterize(park_polygon, template_raster_low)

# Rasterize to high resolution template raster 
polygon_raster_high<- rasterize(park_polygon, template_raster_high)
...
template_raster <- rast(ext(cycle_hire_osm_projected), resolution = 1000, crs = crs(cycle_hire_osm_projected))
```
...
ch_raster1 <- rasterize(cycle_hire_osm_projected, template_raster)
...
```{r}
ch_raster2 <- rasterize(cycle_hire_osm_projected, template_raster, fun = "length")
...
```{r}
ch_raster3 <- rasterize(cycle_hire_osm_projected, template_raster, field = "capacity", fun = sum, na.rm = TRUE)
```
...
```{r}
#| message: false
# Dedine NDVI as the normalized difference between NIR and red bands 

ndvi_fun <- function(nir, red){
  (nir - red)/ (nir + red)
}

# Apply NDVI to LANDS bands 3 & 4 
...
ndvi_map <- tm_shape(ndvi_rast) +
  tm_raster(col.scale = tm_scale_continuous(values = "brewer.rd_yl_gn"), 
            col.legend = tm_legend(orientation = "portrait")) + 
  tm_title(text = "NDVI")
...
```{r}
#| message: false
# Define NDWI as normalized difference between green and nir bands 
...
Normalized Difference Moisture Index: vegetation index used in RS to measure and monitor water content in vegetation 

NDMI = (NIR - SWIR)/ (NIR + SWIR)

- measuring mositure within plant levels: 
higher values = more water content in vegetation 

* Useful for drought monitoring and fire risk 
```{r}
# Define NDMI as normalized difference between nir and swir bands 
ndmi_fun <- function(nir, swir){
  (nir - swir)/ (nir + swir)
}

# Apply NDWI function to landsat bands 3 & 4 

ndmi_rast <- lapp(L7[[c(4,5)]], fun = ndmi_fun)

map_ndmi <- tm_shape(ndmi_rast) + 
  tm_raster(
    col.scale = tm_scale_continuous(values = "brewer.blues"), 
    col.legend = tm_legend(title = "NDMI", 
                           orientation = "portrait")
  ) + 
  tm_title(text = "NDMI")
