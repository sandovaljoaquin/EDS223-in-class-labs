...
# Filter for the region of Canterbury 
...
```{r}
# Subset heights to just features that intersect Canterbury
...
```{r}
# Lave blank space because we're not selecting for columns 

c_height_disjoint <- nz_height[canterbury, , op = st_disjoint]

# Check that intersect is opposite disjoint 
...
# Find heights that are within 1000 km of Canterbury 

nz_height_logical <- st_is_within_distance(nz_height, canterbury,
                                           dis = units::set_units(1000, "km"),
                                           sparse = FALSE) # Return as a logical vector
...
# Specify join based on geometries 
...
cycle_hire_oms <- spData:: cycle_hire_osm
```

```{r}
# Check whether or not points overlap 

if(any(st_intersects(cycle_hire, cycle_hire_oms, sparse = FALSE)) == TRUE) {
  print("points overlap")
} else {
  warning("points dont overlap")
}
```
...
```{r}
# Aggregate values for single points in cycle hire
...
# Select just the value that we want to aggregate 

incongruent_2 <- incongruent |> 
  select(value)

# USe area_weighted interpolation to aggregate the value attribute 
...
```{r}
# Alternative approach: aggregate()

regions2 <- aggregate(x = us_states[, "total_pop_15"], # data to be aggregated 
...
```{r}
# Combine the geometries of western states 
