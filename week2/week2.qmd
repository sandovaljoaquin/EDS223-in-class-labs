...
```{r}
# Create dataframe of attributes about London 
...
```{r}
world_df <- world |> 
  select(-geom)  # Does not actually remove the geom column 
...
colnames(world_df) # Geom is gone
class(world_df) 
ncol(world_df) # One less column 
...
london_proj <- data.frame(x = 53.0000, y = 180000) |> 
...
We can also check if our crs is projected or geographic 
...
tm_shape(world_coffee) +  
  tm_fill("coffee_production_2017", 
          fill.legend = tm_legend(title = "Coffe Production 2017")) 
...
if (nrow(world_coffee_inner) != nrow(coffee_data)) { 
  warning( "Inner join does not match orginal data. Potential data loss during join")}
