...
```{r}
dtm_pre <- rast(here::here("data", "Eaton", "dtm_eaton_alignedNK_1m_PREFIRE.tif"))
dsm_pre <- rast(here::here("data", "Eaton", "dsm_eaton_alignedNK_1m_PREFIRE.tif"))

  
dtm_post <- rast(here::here( "data", "Eaton", "dtm_eaton_epsg32611_geoid12b_1m_POSTFIRE.tif"))
dsm_post <- rast(here::here( "data", "Eaton", "dsm_eaton_epsg32611_geoid12b_1m_POSTFIRE.tif"))
```

```{r}
damage <- st_read(here::here("data","Eaton","06061784-f911-4d0e-aca5-0768ac66aac5.gdb")) %>% st_transform(crs = crs(dsm_post))
```


```{r}

eaton_perimeter <- st_read(here::here( "data", "Eaton","Eaton_Perimeter_20250121", "Eaton_Perimeter_20250121.shp"))%>% st_transform(crs = crs(dsm_post))
```

```{r}
# Check if all dtatsets have matching crs 

if (crs(dtm_pre) == crs(dsm_pre) &
    crs(dsm_pre) == crs(dsm_post) & 
...
```{r}
# Crop surface change to extent of Eaton Perimeter

diff_dsm_crop <- crop(diff_dsm, eaton_perimeter)

# Calculate quantiles for determining breaks 

quantiles <- global(diff_dsm_crop, 
                    fun = quantile, 
                    probs = c(.005, .995), 
                              na.rm = TRUE)

# Remove values outside of upper and lower quartile 
...
tm_shape(diff_dsm_scaled) + 
  ...
  tm_borders(col = "red", 
             linewidth = 2.5) + 
  tm_title("Eaton Fire: DSM Elavation Change (2025-2016") + 
...
elev_mat |> 
  # Create black and white hillshade effect 
  sphere_shade(texture = "bw", 
               sunangle = 315) |> 
  add_overlay(overlay, alphalayer = 0.7) |> 
  # Add realistic shadows at 50% intensity to enhance 3D effect 
...
# Extract minimum elevation loss for each building buffer 
...
```{r}
# Create a plot of elevation loss based on year built 
...
ggplot(damage_plot, 
  ...
  coord_flip() + 
  labs(title = "Building time period vs Elevation loss", 
