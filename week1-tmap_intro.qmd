...
tm_shape(world_vector) + # Defines the spatial object to plot
  tm_polygons()          # How to plot the object
...
tm_shape(world_vector) + # Defines the spatial object to plot
  ...
  tm_dots()                # Defines how to plot the second plot
```
...
tm_shape(world_cities) + 
  tm_dots() + 
  tm_text("name", size = 0.4, col = "darkolivegreen")
...
tm_shape(world_vector) + 
  tm_borders() + 
tm_shape(world_cities, is.main = TRUE) + # plotting world cities last, but set as main shape 
  tm_dots() + 
  tm_text("name")
...
```{r}
tm_shape(world_elevation, bbox = c(-15, 35, 45, 65)) + # Add bounding box to restrict extent of map
  tm_raster() + 
tm_shape(world_vector) + 
  tm_borders()
```
...
tm_shape(world_elevation, bbox = world_cities) + # Bounding box = extent of world cities 
  tm_raster() + 
tm_shape(world_vector) + 
  tm_borders()
...
tm_shape(world_elevation, bbox = "Europe") + 
  tm_raster() + 
  tm_shape(world_vector) + 
  tm_borders()
...
```{r}
# Fill polygon with solid colors 
...
```{r}
# Fill ploygon with a color palette representing a variable
...
```{r}
tm_shape(world_cities) + 
 # tm_squares()  or alternatively do: 
# tm_symbols(shape = 22)
#  tm_bubbles()
  tm_dots()
```
...
tm_shape(world_vector) +
  tm_polygons(# fill = "wb_income_region",
        ...
        #  fill.scale = tm.scale(breaks = c(0, 5000, 10000, 20000, 40000, 80000)) + 
        fill.scale = tm_scale(breaks = c(0, 5000, 20000, 50000, Inf), 
...
tmap_save(map1, here::here("week1", "tmap-example.png"))
